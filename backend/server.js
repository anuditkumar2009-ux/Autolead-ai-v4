...
app.post(
 ...
 async(req,res)=>{
  ...
  try{
   ...
   const prompt=`You are AutoLead AI, a real-estate sales copilot.
...
${JSON.stringify(lead)}`;
   ...
   const score=Number.isInteger(a.score)
    ?Math.max(0,Math.min(100,a.score))
    :null;

   const temp=['Hot','Warm','Cold'].includes(
    a.temperature
   )
    ?a.temperature
    :null;
